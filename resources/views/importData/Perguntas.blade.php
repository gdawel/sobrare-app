        ...
        <div class="row bg-slate-200 m-12 ">
            ...
            <div class="border-solid border-2 border-indigo-600 p-6">
                ...
                <h1 class="font-sans text-xl text-gray-800 text-center">
                    Importação da Tabela de Perguntas<br>.....<br><br></h1>
                

                <div class=" font-sans text-md text-gray-800 text-center">
                        <form action="{{ url('import/perguntas') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="flex-auto">
                                ...
                                <button type="submit" class="float-right bg-blue-500 hover:bg-blue-700 text-white text-sm    
                                        ...
                                        focus:shadow-outline" type="button">Importar Perguntas
                                </button>
                            </div>
                            <p class="text-sm text-center italic"><br><br> O arquivo a ser selecionado nesta tela deve ter apenas a Tabela de Perguntas.</p>
                        </form>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-blue-100 dark:text-blue-100">
            <thead class="text-xs text-white bg-blue-600 dark:text-white">
                    <tr>
                        <th>ID</th>
                        <th>Cod.Teste</th>
                        <th>Cod.Gr.Opc.Resp.</th>
                        <th>N. Pergunta</th>
                        <th>Enunciado</th>
                        <th>Sexo</th>
                        <th>Grupo Opções de Respostas</th>
            ...
            <tbody>
                                ...
                                @foreach ($listarperguntas as $pergunta)
                                <tr class="bg-blue-500 border-b border-blue-400 text-xs">
                                    <td>{{ $pergunta->id }}</td>
                                    <td>{{ $pergunta->testes_id }}</td>
                                    <td>{{ $pergunta->grupo_opcoes_respostas_id }}</td>
                                    <td>{{ $pergunta->sequencia }}</td>
                                    <td>{{ $pergunta->enunciado }}</td>
                                    <td>{{ $pergunta->sexo }}</td>
                                    <td>{{ $pergunta->codGrupoOpcRespostas }}</td>
